    ...
    try:
        async with httpx.AsyncClient(timeout=15) as c:
            if action == "status":
                ...
                await c.post(f"{ORCHESTRATOR_URL}/api/v1/action/restart", json={"target": target})
                return f"Restarting {target}."
            elif action == "backup":
                await c.post(f"{ORCHESTRATOR_URL}/api/v1/action/backup", json={"target": target or "all"})
                return f"Backup triggered for {target or 'all services'}."
            elif action == "deploy" and target:
                await c.post(f"{ORCHESTRATOR_URL}/api/v1/action/deploy", json={"target": target})
                return f"Deploying {target}."
            elif action == "rotate":
                await c.post(f"{ORCHESTRATOR_URL}/api/v1/action/rotate-secrets", json={"target": target or "all"})
