'''Async SDK client for omni-agent-memory-bus.'''
...
class AgentMemoryBusClient:
    '''Simple async SDK client for inter-service communication.'''

    def __init__(self, base_url: str = "http://omni-agent-memory-bus:9853", timeout: float = 30.0):
    ...
    async def primary(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        return await self.request("POST", "/api/v1/sessions", payload or {})
