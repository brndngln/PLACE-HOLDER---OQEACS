'''Async SDK client for omni-crypto-harness.'''
...
class CryptoHarnessClient:
    '''Simple async SDK client for inter-service communication.'''

    def __init__(self, base_url: str = "http://omni-crypto-harness:9828", timeout: float = 30.0):
    ...
    async def primary(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        return await self.request("POST", "/api/v1/crypto/verify", payload or {})
