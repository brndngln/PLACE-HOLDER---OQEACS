'''Async SDK client for omni-elite-error-pattern-intel.'''
...
class EliteErrorPatternIntelClient:
    '''Simple async SDK client for inter-service communication.'''

    def __init__(self, base_url: str = "http://omni-elite-error-pattern-intel:9902", timeout: float = 30.0):
    ...
    async def primary(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        return await self.request("POST", "/api/v1/error-patterns/ingest", payload or {})
