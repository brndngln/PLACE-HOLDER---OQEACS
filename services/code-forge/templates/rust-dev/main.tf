...
resource "coder_agent" "main" {
  ...
  startup_script = <<-STARTUP
    ...
    sudo apt-get install -y -qq \
      ...
      cmake lldb gdb valgrind > /dev/null 2>&1
    ...
    rustup update ${var.rust_channel}
    ...
    cargo install --quiet cargo-edit 2>/dev/null || true
    cargo install --quiet cargo-expand 2>/dev/null || true
    cargo install --quiet cargo-audit 2>/dev/null || true
    cargo install --quiet cargo-tarpaulin 2>/dev/null || true
    cargo install --quiet cargo-nextest 2>/dev/null || true
    cargo install --quiet sccache 2>/dev/null || true
  ...
  metadata {
    display_name = "Cargo Version"
    key          = "cargo_version"
    script       = "source $HOME/.cargo/env 2>/dev/null; cargo --version 2>&1 | awk '{print $2}'"
    interval     = 3600
    timeout      = 5
  }
  ...
  metadata {
    ...
    key          = "rust_toolchain"
    script       = "source $HOME/.cargo/env 2>/dev/null; rustup show active-toolchain 2>&1 | awk '{print $1}'"
    interval     = 3600
    timeout      = 5
  }
