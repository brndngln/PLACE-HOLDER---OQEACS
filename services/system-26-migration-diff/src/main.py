'''omni-migration-diff service entrypoint.'''
...
REQUEST_COUNT = Counter(
    "omni_migration_diff_requests_total",
    "Total requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "omni_migration_diff_request_seconds",
    "Request latency",
    ["method", "endpoint"],
)

ENDPOINTS: list[dict[str, str]] = [{"method": "POST", "path": "/api/v1/migration/ingest", "operation": "migration_ingest_post"}, {"method": "GET", "path": "/api/v1/migration/check", "operation": "migration_check_get"}, {"method": "POST", "path": "/api/v1/migration/sync", "operation": "migration_sync_post"}]
...
app = FastAPI(title="omni-migration-diff", version="1.0.0", lifespan=lifespan)
