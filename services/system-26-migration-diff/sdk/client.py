'''Async SDK client for omni-migration-diff.'''
...
class MigrationDiffClient:
    '''Simple async SDK client for inter-service communication.'''

    def __init__(self, base_url: str = "http://omni-migration-diff:9843", timeout: float = 30.0):
    ...
    async def primary(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        return await self.request("POST", "/api/v1/migration/ingest", payload or {})
