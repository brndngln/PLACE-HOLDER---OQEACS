'''Async SDK client for omni-native-fuzz-cluster.'''
...
class NativeFuzzClusterClient:
    '''Simple async SDK client for inter-service communication.'''

    def __init__(self, base_url: str = "http://omni-native-fuzz-cluster:9826", timeout: float = 30.0):
    ...
    async def primary(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        return await self.request("POST", "/api/v1/fuzz/native/start", payload or {})
