'''Async SDK client for omni-k8s-target.'''
...
class K8sTargetClient:
    '''Simple async SDK client for inter-service communication.'''

    def __init__(self, base_url: str = "http://omni-k8s-target:9891", timeout: float = 30.0):
    ...
    async def primary(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        return await self.request("POST", "/api/v1/k8s/generate-helm", payload or {})
