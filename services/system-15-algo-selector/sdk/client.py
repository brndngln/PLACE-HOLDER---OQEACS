'''Async SDK client for omni-algo-selector.'''
...
class AlgoSelectorClient:
    '''Simple async SDK client for inter-service communication.'''

    def __init__(self, base_url: str = "http://omni-algo-selector:9808", timeout: float = 30.0):
    ...
    async def primary(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        return await self.request("POST", "/api/v1/benchmark/run", payload or {})
