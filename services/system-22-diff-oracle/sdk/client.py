'''Async SDK client for omni-diff-oracle.'''
...
class DiffOracleClient:
    '''Simple async SDK client for inter-service communication.'''

    def __init__(self, base_url: str = "http://omni-diff-oracle:9827", timeout: float = 30.0):
    ...
    async def primary(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        return await self.request("POST", "/api/v1/diff/record", payload or {})
